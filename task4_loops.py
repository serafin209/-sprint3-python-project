...
for game_year in video_game_sales:
    
    if game_year[YEAR] < 2000:
...
for NA_JP_sales in video_game_sales:
    
    na_gs = NA_JP_sales[NA_SALES] + na_gs
    
    jp_gs = NA_JP_sales[JP_SALES] + jp_gs
...
if na_gs > jp_gs:
    ...
    print (f"North America had higher sales with a Total NA sales: {na_gs}")
    ...
    print(f"Japan had higher sales with a total JP sales: {jp_gs}")

nintendo_games=[]

for ninte_game in video_game_sales:
    if ninte_game[PUBLISHER] == 'Nintendo':
        nintendo_games.append(ninte_game[NAME])

print(nintendo_games, len(nintendo_games))
